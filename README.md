  ...
  deploy:
    ...
    steps:
      ...
      - name: Deploy Application via SSH
        uses: falling42/ssh-deploy@v0.1.7
        with:
  ...
  push:
    pipeline:
      ...
      stages:
        ...
        - name: Deploy Application via SSH
          image: docker.cnb.cool/falling42/ssh-deploy:v0.1.7
...
* If using `imports`, you must configure `allow_images` to permit `docker.cnb.cool/falling42/ssh-deploy:v0.1.7` and `allow_slugs` for your repository in the secrets vault.
